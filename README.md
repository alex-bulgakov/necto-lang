# Necto Programming Language (v2.0.0 — Pure Self-Hosting & Embedded Toolchain)
...
- **Синтез лучших концепций**:
  - **Встроенный переносимый компилятор Clang (`necto toolchain`)**: многоуровневый поиск бэкенда (`bin/clang`, `.necto/toolchain/`, системный PATH) для автономной сборки без ручной настройки среды.
  - **100% самохостинг и автономность (Stage 1, 2, 3)**: компилятор Necto полностью написан на чистом Necto в пакете [**`compiler/`**](compiler/) и умеет компилировать программы напрямую через встроенный C FFI (`extern "C" { fn system(cmd: str) -> int }`)!
  - **Автономные скрипты сборки без Go**: `bootstrap.ps1` и `bootstrap.bat` для моментального развертывания на чистой машине.
...
- **Проектная система и инструменты**:
  ...
  - `necto toolchain` — проверка и управление встроенным бэкендом компилятора.
